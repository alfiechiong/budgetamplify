...
import {listExpensess} from '../../graphql/queries';
...
export default function StickyHeadTable() {
  ...
  const getExpenses = async()=>{
    try {
    const myExpenses:any = await API.graphql(graphqlOperation(listExpensess))
    const expensesItems = myExpenses.data.listExpensess.items
    console.log(expensesItems)

    const tableExpeses = expensesItems.map((expense:any, i:number)=>{
      return  createData(expense.name, parseFloat(expense.amount), expense.category, expense.date)
    })

    setExpenses(tableExpeses)
  }catch(e){
    ...
    <Paper className={classes.root}>
      <TableContainer className={classes.container}>
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {expenses && expenses.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row:any) => {
              return (
